...
const Login = () => {
    ...
    const functAutentication = async (e) => {
        ...
        const videojuego_favorito = e.target.videojuego_favorito ? e.target.videojuego_favorito.value : '';

        if (registrando) {
            const videojuegosCollection = collection(db, "videojuegos");
            const videojuegoSnapshot = await getDocs(query(videojuegosCollection, where("titulo", "==", videojuego_favorito)));

            if (videojuegoSnapshot.empty) {
                alert("El videojuego favorito ingresado no existe en nuestra base de datos. Por favor, ingresa un videojuego válido.");
                return;
            }
                ...
                <div className='col-md-4'>
                        <div className="padre" >
                            <div className="card card-body shadow-lg">
                                <div className='estilo-titulo'>{registrando ? "Adelante, registrate con nosotros!" : "Bienvenido a tu comunidad, gamer!"}</div>
                                <img src = {ImagenPerf} alt="Imagen de Perfil" className='estilo-profile' />
                                    <form onSubmit={functAutentication}>
                                        ...
                                        {registrando && (
                                            <>
                                                <input type="text" placeholder="Ingresar nombre" className='cajatexto' id='nombre'/>
                                                <input type="text" placeholder="Ingresar apellido" className='cajatexto' id='apellido'/>
                                                <input type="text" placeholder="Ingresar nombre de usuario" className='cajatexto' id='username'/>
                                                <input type="text" placeholder="Ingresar videojuego favorito" className='cajatexto' id='videojuego_favorito'/>
                                            </>
                                        )}
