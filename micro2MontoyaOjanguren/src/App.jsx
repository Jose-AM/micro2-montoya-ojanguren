...
async function initializeApp() {
  ...
  for (const videojuego of data_videojuegos) {
    const videojuegoSnapshot = await getDocs(
      query(videojuegosCollection, where('titulo', '==', videojuego.titulo)),
    )

    if (videojuegoSnapshot.empty) {
      try {
        await addDoc(videojuegosCollection, videojuego)
  ...
  for (const club of data_clubes) {
    const clubSnapshot = await getDocs(
      query(clubesCollection, where('nombre', '==', club.nombre)),
    )

    if (clubSnapshot.empty) {
      try {
        await addDoc(clubesCollection, club)
        console.log('Document successfully written!')
      } catch (error) {
        console.error('Error writing document: ', error)
      }
...
function App() {
  ...
  return (
    <Routes>
      ...
      <Route path="/clubs" element={<div>Video juegos</div>} />
      <Route path="/profile/:id" element={<div>Video juegos</div>} />
